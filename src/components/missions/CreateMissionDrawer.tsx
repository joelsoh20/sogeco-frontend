...
import { AlertTriangle, MapPinned, Package, Route, Truck } from 'lucide-react';
...
import type { City, CreateMissionRequest } from '@/types/api';
...
interface CreateMissionDrawerProps {
  open: boolean;
  onClose: () => void;
  /** Pre-selectionne et verrouille le chauffeur — ouverture depuis la fiche chauffeur ("voyage precis"). */
 ...
 *
 * Meme trou que "Ajouter un camion" : le bouton existait depuis la
 * premiere livraison du frontend sans jamais avoir ete raccorde.
 *
 * Point d'attention : les dates viennent d'un champ datetime-local,
 * qui ne porte aucun fuseau horaire — le navigateur pourrait tourner
 * dans n'importe quel fuseau, meme si l'utilisateur est au Cameroun.
 * On force donc explicitement +01:00 (Afrique/Douala, sans heure
 * d'ete) plutot que de laisser le navigateur deviner.
 *
 * Distance et carburant affiches ici sont une ESTIMATION en aide a la
 * saisie (GET /missions/estimate) — ils ne sont pas envoyes dans le
 * formulaire : la distance persistee reste calculee cote serveur selon
 * la meme logique de repli (corridor > OpenRouteService > vol d'oiseau
 * sites/villes), et la
 * consommation reelle vient des pleins de carburant, jamais d'un calcul
 * a priori.
 */
export function CreateMissionDrawer({ open, onClose, initialDriverId, initialDriverName }: CreateMissionDrawerProps) {
  ...
  useEffect(() => {
    if (open) {
      reset(initialDriverId ? { driverId: String(initialDriverId) } : undefined);
      setMapTarget('origin');
    }
  ...
  // Le camion doit etre base dans la ville de depart — tant qu'elle n'est pas encore
  // choisie (le champ ville vient plus bas dans le formulaire), la flotte entiere reste
  // proposee plutot que de bloquer le champ camion.
  const vehiclesForCity = originCityId
    ? (vehicles.data?.content ?? []).filter((v) => String(v.cityId) === originCityId)
  ...
  // Chaque camion a un chauffeur attitre (affectation standing) : le pre-selectionner evite
  // une double saisie, sans empecher de le changer si ce chauffeur n'est pas disponible.
  useEffect(() => {
    if (initialDriverId || !vehicleId) return;
  ...
  // Un simple signal, jamais bloquant — le dispatcheur peut toujours prevoir
  // un ravitaillement en route.
  ...
  const create = useMutation({
    mutationFn: (payload: CreateMissionRequest) => missionApi.create(payload),
    onSuccess: (mission) => {
      toast.success(t('missionForm.createSuccess', { number: mission.missionNumber }));
  ...
  return (
    <Drawer
      open={open}
      onClose={handleClose}
      title={t('missionForm.createTitle')}
      subtitle={initialDriverName ? t('missionForm.tripForDriver', { name: initialDriverName }) : t('missionForm.planSubtitle')}
    >
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            ...
            {errors.vehicleId
              ...
              : originCityId && vehiclesForCity.length === 0 && (
                <p className="mt-1 text-xs text-slate-400">{t('missionForm.noVehicleHint')}</p>
              )}
          </div>
          <div>
            ...
            <select
              ...
              {...register('driverId', { required: t('common.requiredField') })}
            >
              <option value="">{t('common.selectPlaceholder')}</option>
              {drivers.data?.content.map((d) => (
                <option key={d.id} value={d.id}>{d.fullName}</option>
              ))}
              {initialDriverId && !drivers.data?.content.some((d) => d.id === initialDriverId) && (
                <option value={initialDriverId}>{initialDriverName}</option>
              )}
            </select>
          ...
          <>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                {errors.destinationCityId && <p className="mt-1 text-xs text-red-600 dark:text-red-400">{errors.destinationCityId.message}</p>}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                {errors.destinationAgencyId && <p className="mt-1 text-xs text-red-600 dark:text-red-400">{errors.destinationAgencyId.message}</p>}
              </div>
            </div>
        ...
        {/* Estimation : aide a la saisie, pas une valeur enregistree — voir GET /missions/estimate */}
        ...
        {/* Signal, jamais bloquant : le dispatcheur reste libre de creer la mission et de prevoir un ravitaillement en route. */}
        {insufficientFuel && (
