...
import type { Claim, ClaimRequest, ClaimType } from '@/types/compliance';
...
interface EditClaimDrawerProps {
  open: boolean;
  onClose: () => void;
  claim: Claim;
}

/** Modification d'un sinistre dans les 24h suivant sa declaration (RG-8-EDIT) — memes champs que la creation, pre-remplis. */
export function EditClaimDrawer({ open, onClose, claim }: EditClaimDrawerProps) {
  const { t } = useTranslation();
  const queryClient = useQueryClient();
  const [error, setError] = useState<string | null>(null);

  const vehicles = useQuery({ queryKey: ['vehicles', 'list-for-claim'], queryFn: () => vehicleApi.list(0, 100), enabled: open });
  ...
  useEffect(() => {
    if (open) {
      reset({
        ...
        estimatedCost: claim.estimatedCost != null ? String(claim.estimatedCost) : '',
      });
    }
  ...
  const update = useMutation({
    mutationFn: (payload: ClaimRequest) => complianceApi.updateClaim(claim.id, payload),
    onSuccess: () => {
      toast.success(t('claimForm.editSuccess'));
  ...
  const onSubmit = (values: FormValues) => {
    ...
    update.mutate({
      ...
      // Le formulaire ne touche pas au chauffeur ni a la police — on reconduit les valeurs existantes.
      driverId: claim.driverId ?? undefined,
      insurancePolicyId: claim.policyId ?? undefined,
  ...
  return (
    <Drawer open={open} onClose={onClose} title={t('claimForm.editTitle')} subtitle={claim.claimNumber}>
