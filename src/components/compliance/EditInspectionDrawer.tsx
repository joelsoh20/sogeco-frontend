...
import type { InspectionResult, TechnicalInspection, TechnicalInspectionRequest } from '@/types/compliance';
...
export function EditInspectionDrawer({ open, onClose, inspection }: EditInspectionDrawerProps) {
  const { t } = useTranslation();
  const queryClient = useQueryClient();
  const [error, setError] = useState<string | null>(null);

  const vehicles = useQuery({ queryKey: ['vehicles', 'list-for-inspection'], queryFn: () => vehicleApi.list(0, 100), enabled: open });
  const centers = useQuery({ queryKey: ['partners', 'CENTRE_VISITE'], queryFn: () => partnerApi.active('CENTRE_VISITE'), enabled: open });
  ...
  useEffect(() => {
    if (open) {
      reset({
        ...
        partnerId: inspection.centerId != null ? inspection.centerId.toString() : '',
        inspectionDate: inspection.inspectionDate,
        nextInspectionDate: inspection.nextInspectionDate ?? '',
        result: inspection.result,
        defectsNoted: inspection.defectsNoted ?? '',
        cost: inspection.cost != null ? String(inspection.cost) : '',
      });
    }
  ...
  const update = useMutation({
    mutationFn: (payload: TechnicalInspectionRequest) => complianceApi.updateInspection(inspection.id, payload),
    onSuccess: () => {
      toast.success(t('inspectionForm.editSuccess'));
  ...
  return (
    <Drawer open={open} onClose={onClose} title={t('inspectionForm.editTitle')} subtitle={inspection.registrationNumber}>
