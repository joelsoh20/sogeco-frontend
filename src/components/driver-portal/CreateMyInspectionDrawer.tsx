...
/** Variante simplifiee de CreateInspectionDrawer pour l'espace chauffeur : le camion est impose, pas choisi. */
export function CreateMyInspectionDrawer({ open, onClose, vehicleId, registrationNumber }: Props) {
  ...
  return (
    <Drawer open={open} onClose={onClose} title={t('inspectionForm.title')} subtitle={t('carteGriseForm.vehicleSubtitle', { registration: registrationNumber })}>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            ...
            {errors.inspectionDate && <p className="mt-1 text-xs text-red-600">{errors.inspectionDate.message}</p>}
          </div>
          <div>
        ...
        <p className="text-xs text-slate-500">
          {t('inspectionForm.immutableHint')}
        </p>
